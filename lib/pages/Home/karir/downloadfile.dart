...
class HalamanDownload extends StatefulWidget with WidgetsBindingObserver {
  final TargetPlatform platform;

  HalamanDownload({Key key, this.title, this.namalengkap, this.platform})
      : super(key: key);

  final String title, namalengkap;

  @override
  _HalamanDownloadState createState() => new _HalamanDownloadState();
}
...
class _HalamanDownloadState extends State<HalamanDownload> {
  ...
  void initState() {
    ...
    _prepare();
    document(widget.namalengkap.toString());
  }

  var _documents = [];
  void document(namalengkap2) {
    setState(() {
      final documents = [
        {
          'name': 'CV pelamar ${namalengkap2.replaceAll(' ', '-')}',
          'link':
              'https://edunitas.com/file/educareer_pelamar_${namalengkap2.replaceAll(' ', '-')}.pdf'
        },
      ];
  ...
  Future<Null> _prepare() async {
    ...
    _items.add(_ItemHolder(name: 'Curriculum vitae'));
...
class DownloadItem extends StatelessWidget {
  ...
  Widget _buildActionForTask(_TaskInfo task) {
    ...
    if (task.status == DownloadTaskStatus.undefined) {
      ...
      return Text('Dibatalkan', style: TextStyle(color: Colors.red));
    } else if (task.status == DownloadTaskStatus.failed) {
      return Row(
        mainAxisSize: MainAxisSize.min,
        mainAxisAlignment: MainAxisAlignment.end,
      ...
      return Text('Menunggu', style: TextStyle(color: Colors.orange));
    } else {
      return null;
    }
  }
